;(function () {
	...
	const currentScript = document.currentScript || document.querySelector('script[data-shop-token]')
	...
	const SESSION_MODAL_COPY_RU = {
		message: 'У вас уже есть активная примерка. Закройте текущую или дождитесь её окончания, чтобы начать новую.',
		ok: 'Понятно',
	}
	...
	const WIDGET_CONFIG = {
		...
		openCardSelector:
			'.t-store__prod-popup.t-popup_show, .t-store__prod-popup_showed, .t-popup_show .t-store__prod-popup, .t-popup_show .js-store-prod-all, .t-popup_show .js-product-single-wrapper, .t-popup_show .t-store__prod-popup__container, .t-catalog__prod-popup.t-popup_show, .t-catalog__prod-popup_showed, .t-popup_show .t-catalog__prod-popup, .t-popup_show .js-catalog-prod-all, .t-popup_show .t-catalog__prod-popup__container, .t-popup_show .js-catalog-product, .t-catalog__product-popup.t-popup_show, .t-popup_show .t-catalog__product-popup',
		...
		excludedProductNameKeywords: ['Сертификат', 'Бокс', 'Пакет', 'Упаковка', 'Игры', 'Дженга'],
	...
	function loadShopConfig() {
		...
		var url =
			WIDGET_CONFIG.widgetUrl.replace(/\/$/, '') +
			'/api/widget/config?shop_token=' +
			encodeURIComponent(WIDGET_CONFIG.shopToken)
		return fetch(url)
	...
	function updateMinimizerButton(isMinimized, attentionState) {
		...
		btn.classList.toggle('virtual-fitting-toggle--generating', isMinimized && !showingAttention && generationInProgress)
		if (attentionState === 'success') {
			btn.innerHTML =
				'<span class="virtual-fitting-toggle-check">✓</span><span class="virtual-fitting-toggle-badge"></span>'
		} else if (attentionState === 'error') {
			btn.innerHTML =
				'<span class="virtual-fitting-toggle-cross">×</span><span class="virtual-fitting-toggle-badge"></span>'
		} else if (isMinimized) {
			btn.innerHTML = generationInProgress
				? MINIMIZER_SPINNER_HTML
				: '<span class="virtual-fitting-toggle-letter">L</span>'
		}
	...
	function createIframe() {
		...
		// );
		iframe.setAttribute('allow', 'camera; microphone; web-share; clipboard-read; clipboard-write')
	...
	function resolveFirstPhotoElement(contextElement) {
		const openRoot =
			getOpenCardRoot(contextElement) || (contextElement && contextElement.nodeType === 1 ? contextElement : null)
	...
	function resolveButtonHostElement(targetImageElement, productElement) {
		...
		if (targetImageElement && targetImageElement.closest) {
			...
			if (zoomSlide && zoomSlide.hasAttribute && zoomSlide.hasAttribute('on:click')) {
		...
		if (productElement && productElement.closest) {
			pushContext(
				productElement.closest(
					'.js-store-prod-all, .js-catalog-prod-all, .js-product-single-wrapper, .t-store__prod-popup, .t-catalog__prod-popup',
				),
			)
		}
	...
	function resolveProductPrice(productElement) {
		...
		for (let i = 0; i < contexts.length; i++) {
			const ctx = contexts[i]
			const dataPrice = getAttrValue(ctx, 'data-price') || getAttrValue(ctx, 'data-product-price')
	...
	function startStartupRenderRecheck() {
		...
		startupRenderInterval = window.setInterval(() => {
			...
			if (isTildaPopupOnlyMode()) {
				const hasPopupButton = document.querySelector(`.${WIDGET_CONFIG.buttonClass}[${PRODUCT_BTN_ATTR}="true"]`)
				if (hasPopupButton) {
					window.clearInterval(startupRenderInterval)
					startupRenderInterval = null
					return
	...
	function isTildaSingleColor(productElement) {
		...
		var context =
			(productElement &&
				productElement.closest &&
				productElement.closest(
					'.t-store__prod-popup, .t-catalog__prod-popup, .js-store-prod-all, .js-catalog-prod-all, .js-product-single-wrapper',
				)) ||
			document
	...
	function findFirstImage(productElement) {
		...
		for (var i = 0; i < selectors.length; i++) {
			var el = (productElement && productElement.querySelector(selectors[i])) || document.querySelector(selectors[i])
			if (el) {
	...
	function findCurrentImage(productElement) {
		...
		var candidates = (activeSlide || productElement).querySelectorAll(
			'img, .t-bgimg, .t-slds__bgimg, .js-product-img, [data-original], [data-src]',
		)
		var bestSrc = null
		var bestArea = 0
	...
	function extractProductData(productElement) {
		...
		const imageSrc =
			...
			(!isTildaPopupOnlyMode() && isTildaSingleColor(productElement)
				? findFirstImage(productElement)
	...
	function createButton(productElement) {
		...
		button.className = IS_EN_WIDGET ? WIDGET_CONFIG.buttonClass + ' ' + EN_LAUNCHER_MODIFIER : WIDGET_CONFIG.buttonClass
		button.type = 'button'
		if (isTildaPopupOnlyMode()) {
		...
		const imageElement = productElement.querySelector(WIDGET_CONFIG.imageSelector)
		const firstPhotoElement = isOpenCardModeActive(productElement) ? resolveFirstPhotoElement(productElement) : null
	...
	function getLooksyAdapterName() {
		const script = document.currentScript || document.querySelector('script[data-shop-token]')
	...
	function extractBitrixV1ExtendedProductData() {
		const root = document.querySelector('.c-catalog-element[data-data][data-properties]')
		...
		const variants = properties
			.map(property => ({
				...
				values: Object.entries(property.values || {})
					.filter(([id, value]) => id !== '0' && value && value.id !== 0 && value.name !== '-' && value.stub !== true)
					.map(([id, value]) => ({
						id: String(id),
						name: value.name,
		...
		const extractOfferPrice = offer => {
			...
			const source = price.discount && price.discount.use ? price.discount : price.base
	...
	function performBitrixV1AddToCart(payload, reply) {
		...
		const button = document.querySelector(
			'.intec-ui-control-basket-button[data-basket-id="' + offerId + '"][data-basket-action="add"]',
		)
